...
function renderSubjectsTable() {
    ...
    let html = '';
    subjects.forEach(subject => {
        const studentCount = students.filter(s => s.subjects?.includes(subject.code)).length;
...
async function pushAnnouncement(subjectCode, subjectName) {
    ...
    try {
        ...
        const enrolledStudents = students.filter(s => s.subjects?.includes(subjectCode));
...
async function editStudent(studentId) {
    try {
        ...
        if (student) {
            ...
            document.querySelectorAll('#subjectCheckboxes input').forEach(cb => {
                cb.checked = student.subjects?.includes(cb.value) || false;
            });
...
async function loadAttendance() {
    ...
    try {
        ...
        if (selectedSubject) {
            ...
            visibleStudents = studentsList.filter(s => s.subjects?.includes(selectedSubject));
    ...
    try {
        ...
        if (status === 'absent' || status === 'late') {
            ...
            await database.ref(`notifications/${studentId}`).push({
                ...
                message: `${subjectCode}: ${status} on ${new Date(date).toLocaleDateString()}`,
                type: status,
                subjectCode: subjectCode,
                time: time,
                timestamp: Date.now(),
...
async function markAllStudents(status) {
    ...
    try {
        const studentsList = students.filter(s => s.subjects?.includes(subjectCode));
        
        for (const student of studentsList) {
            await updateAttendance(student.id, student.name, subjectCode, status);
        }
...
function filterStudentsBySubject() {
    ...
    // This would need to be implemented with the student data
    // For now, just reload students
    loadStudents();
}
